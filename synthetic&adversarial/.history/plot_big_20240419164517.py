...
def plot_test_toy_utility(ax):
    ...
    ax2.tick_params(axis='both',          # 应用到y轴
               ...
               labelleft=False,   # 隐藏左侧刻度标签
               labelright=False)   # 显示右侧刻度标签
    ax.set_title('Validation', fontsize=front_size, fontweight='bold')
    ax.yaxis.set_label_position("right")
    ax.set_xlabel('Feature 1', fontsize=front_size)
    ax.set_ylabel('Feature 2', fontsize=front_size, rotation=270, labelpad=25)
...
def plot_influence_inner_product_LR_Utility(ax):
    ...
    ax2.tick_params(axis='both',          # 应用到y轴
               ...
               labelleft=False,   # 隐藏左侧刻度标签
               labelright=False)   # 显示右侧刻度标签
    ax.set_title('Influence Estimate', fontsize=front_size, fontweight='bold')
    ax.yaxis.set_label_position("right")
...
def scale_tick(x, pos):
    """将刻度值乘以4"""
    return '%1.1f' % (0.25 + x * 3.5)
...
def plot_training_half_moon (ax):
    ...
    # # 设置刻度格式化器以乘以4
    # ax.xaxis.set_major_formatter(FuncFormatter(scale_tick))
    # ax.yaxis.set_major_formatter(FuncFormatter(scale_tick))
    ...
    #plt.axis('equal')
    # Adjust layout to make room for the full feature 2 label
    #ax.xaxis.set_major_locator(ticker.FixedLocator([-0.0714]))
    ax.set_xticks([-1.214285, -0.642857,-0.071428,0.5,1.071428,1.642857,2.214285])
    ax.set_xticklabels([-4,-2,0,2,4,6,8])
    ax.set_yticks([-0.84375,-0.53125,-0.21875,0.09375,0.40625,0.71875,1.03125,1.34375])
    ax.set_yticklabels([-6,-4,-2,0,2,4,6,8])
    #ax.yaxis.set_major_locator(ticker.FixedLocator([-0.75,-0.25,0.5,1]))
    plt.tight_layout()
...
fig, axs = plt.subplots(2, 3, figsize=(20, 14))  # figsize可根据需要调整
...
fig.subplots_adjust(bottom=0.10, wspace=0.2, hspace=0.2)
