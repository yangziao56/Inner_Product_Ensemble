...
def train_model(args, data, sample_weight=None):
    tik = time.time()
...
def reweight_points(args, data, I2):
    ...
    return trimmed_data
'''    
def reweight_points(args, data, I2, tau=1.0):
    trimmed_data = copy.deepcopy(data)
    ...
    # 归一化影响分数
    normalized_I2 = (I2 - mu_I2) / np.sqrt(sigma_I2**2 + epsilon)
    #normalized_I2 = np.clip(I2, -1, 1)
    #print("normalized_I2", normalized_I2)
    # 计算权重
    exp_scores = np.exp(tau * normalized_I2)
    w_train = exp_scores / np.sum(exp_scores)
    w_train = w_train * len(w_train) *2
    #w_train = np.clip(w_train, 0, 2)
...
if __name__ == "__main__":
    ...
    for _ in range(n_iters):
        ...
        if args.task == 'reweight':
            #REWEIGHT
            weight = reweight_points(args, data, influences)
            weight_IP = reweight_points(args, data, IP)
            weight_IP_ensemble = reweight_points(args, data, IP_ensemble)
            weight_Gradient_Tracing = reweight_points(args, data, Gradient_Tracing)
            weight_datainf = reweight_points(args, data, Datainf)
            weight_Self_Gradient_Tracing = reweight_points(args, data, Self_Gradient_Tracing)
            weight_Self_datainf = reweight_points(args, data, Self_datainf)
            weight_self_LiSSA = reweight_points(args, data, Self_Lissa)
            weight_EKFAC = reweight_points(args, data, EKFAC)

            trimmed_data = data
            trimmed_data_IP = data
            trimmed_data_IP_ensemble = data
            trimmed_data_Gradient_Tracing = data
            trimmed_data_datainf = data
            trimmed_data_Self_Gradient_Tracing = data
            trimmed_data_Self_datainf = data
            trimmed_data_self_LiSSA = data
            trimmed_data_EKFAC = data
            ...
            weight = None
            weight_IP = None
            weight_IP_ensemble = None
            weight_Gradient_Tracing = None
            weight_datainf = None
            weight_Self_Gradient_Tracing = None
            weight_Self_datainf = None
            weight_self_LiSSA = None
            weight_EKFAC = None
            ...
            if args.task == 'remove':
                ...
                #relabel
        defense_model = train_model(args, trimmed_data, sample_weight=weight)
        defense_model_IP = train_model(args, trimmed_data_IP, sample_weight=weight_IP)
        defense_model_IP_ensemble = train_model(args, trimmed_data_IP_ensemble, sample_weight=weight_IP_ensemble)
        defense_model_Gradient_Tracing = train_model(args, trimmed_data_Gradient_Tracing, sample_weight=weight_Gradient_Tracing)
        defense_model_datainf = train_model(args, trimmed_data_datainf, sample_weight=weight_datainf)
        defense_model_Self_Gradient_Tracing = train_model(args, trimmed_data_Self_Gradient_Tracing, sample_weight=weight_Self_Gradient_Tracing)
        defense_model_Self_datainf = train_model(args, trimmed_data_Self_datainf, sample_weight=weight_Self_datainf)
        defense_model_self_LiSSA = train_model(args, trimmed_data_self_LiSSA, sample_weight=weight_self_LiSSA)
        defense_model_EKFAC = train_model(args, trimmed_data_EKFAC, sample_weight=weight_EKFAC)
