...
def plot_training_half_moon (ax):
    ...
    ax2.tick_params(axis='y',          # 应用到y轴
               ...
               labelleft=False,   # 隐藏左侧刻度标签
               labelright=False)   # 显示右侧刻度标签
    ax2.set_ylabel('Non-Linear', fontsize=front_size,labelpad=40, fontweight='bold')
    ax.yaxis.set_label_position("right")
    ax.set_xlabel('Feature 1', fontsize=front_size)
    ax.set_ylabel('Feature 2', fontsize=front_size, rotation=270, labelpad=25)
...
fig, axs = plt.subplots(2, 3, figsize=(16, 10.12))  # figsize可根据需要调整
...
#     plt.Line2D([0], [0], marker='o', color='w', markerfacecolor='gray', markersize=10, label='Normal Point'),
#     plt.Line2D([0], [0], marker='X', color='w', markerfacecolor='gray', markersize=10, label='Outlier'),
#     plt.Line2D([0], [0], linestyle='-', color='gray', lw=4, label='Majority'),
#     plt.Line2D([0], [0], linestyle='--', color='gray', lw=4, label='Minority')
# ]

# # 添加图例
# fig.legend(loc='upper center', bbox_to_anchor=(0.5, 1.05), ncol=3, fontsize='large')

def plot_training_toy_Utility(ax):
    ax.plot(np.random.rand(10), np.random.rand(10), 'r-', label='Utility Line')
    ax.set_title('Utility Plot')

def plot_test_toy_utility(ax):
    ax.scatter(np.random.rand(10), np.random.rand(10), color='blue', label='Utility Scatter')
    ax.set_title('Utility Scatter Plot')

def plot_influence_inner_product_LR_Utility(ax):
    ax.bar(np.arange(10), np.random.rand(10), color='green', label='Bar Chart')
    ax.set_title('Bar Chart Example')

def plot_training_half_moon(ax):
    ax.hist(np.random.rand(10), color='orange', label='Histogram')
    ax.set_title('Histogram Example')

def plot_test_half_moon(ax):
    ax.pie(np.random.rand(4), labels=['A', 'B', 'C', 'D'], autopct='%1.1f%%')
    ax.set_title('Pie Chart Example')

def plot_influence_inner_product_MLP(ax):
    ax.plot(np.random.rand(10), 'ko-', label='MLP Line')
    ax.set_title('MLP Line Plot')

# 创建一个2x3的子图网格
fig, axs = plt.subplots(2, 3, figsize=(16, 10.12))  # Adjust size as needed
...
# 创建图例
legend_elements = [
    mpatches.Patch(color='red', label='Utility Line'),
    mpatches.Patch(color='blue', label='Utility Scatter'),
    mpatches.Patch(color='green', label='Bar Chart'),
    mpatches.Patch(color='orange', label='Histogram'),
    plt.Line2D([0], [0], marker='o', color='w', markerfacecolor='black', markersize=10, label='MLP Line')
]
fig.legend(handles=legend_elements, loc='upper center', bbox_to_anchor=(0.5, 1.02), ncol=3, fontsize='large')


# 调整子图和画布布局
fig.subplots_adjust(top=0.85)  # 适当调整顶部空间以适应图例
# 调整子图间距
