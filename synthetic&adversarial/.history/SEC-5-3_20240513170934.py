...
def compute_EKFAC(args, data, model):
    ...
    # Initialize the task for Kronfluence.
    # 包装你的逻辑回归模型为 Kronfluence 可用的模型
    class KronfluenceLogisticRegression(nn.Module):
        def __init__(self, logistic_regression_model):
            super(KronfluenceLogisticRegression, self).__init__()
            self.logistic_regression_model = logistic_regression_model

        def forward(self, x):
            pred, _ = self.logistic_regression_model.pred(x.detach().numpy())
    ...
    class LogisticRegressionTask:
        ...
        pass
    # 创建你的逻辑回归模型实例
    logistic_regression_model = LogisticRegression(l2_reg=0.01, fit_intercept=True)
    logistic_regression_model.fit(train_dataset.data.numpy().reshape(-1, 784), train_dataset.targets.numpy())

    # 包装逻辑回归模型为 Kronfluence 可用的模型
    model = KronfluenceLogisticRegression(logistic_regression_model)

    task = LogisticRegressionTask()
    prepared_model = prepare_model(model=model, task=task)
    analyzer = Analyzer(analysis_name=args.dataset, model=prepared_model, task=task)

    # Set DataLoader arguments.
    dataloader_kwargs = DataLoaderKwargs(num_workers=4)
    analyzer.set_dataloader_kwargs(dataloader_kwargs)

    # Compute influence factors using EKFAC.
    start_time = time.time()
    factor_args = FactorArguments(strategy="ekfac")
    analyzer.fit_all_factors(
        factors_name="ekfac",
        dataset=data.x_train,  # Assuming data.x_train is the dataset
        per_device_batch_size=64,  # Adjust batch size as needed
    ...
    analyzer.compute_self_scores(scores_name="self", factors_name="ekfac", score_args=score_args, train_dataset=data.x_train)
    ...
    return -scores  # Returning both the original influence and the EKFAC scores
...
if __name__ == "__main__":
    ...
    for _ in range(n_iters):
        ...
        Self_Gradient_Tracing, Self_datainf = compute_Self(args, data, model)
