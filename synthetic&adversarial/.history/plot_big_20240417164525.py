...
def plot_training_toy_Utility (ax):
    ...
    for i, (a, b) in enumerate(zip(f1, f2)):
        ...
        if influence_values[i] < sorted_infl[10]:
            ...
            if majority_minority[i] == 'Female':  # majority实框
                ...
                ax.scatter(a, b, marker='o', color=y_colormap[y[i]], s=200, edgecolor='black', alpha=alpha_minority)  # 调低对比度
    ...
    for i in drawn_as_outlier:
        if majority_minority[i] == 'Female':  # majority实框
            ...
            ax.scatter(f1[i], f2[i], marker='X', color=y_colormap[y[i]], s=500, edgecolor='black', alpha=alpha_minority)
            #ax.scatter(f1[i], f2[i], marker='X', color=y_colormap[y[i]], s=500, edgecolors='black', linestyle='--', alpha=alpha1)
            # ax.plot(f1[i], f2[i], marker='X', color=y_colormap[y[i]], markersize=22, 
            # linestyle='--', markeredgewidth=2, markeredgecolor='black', 
            # dash_joinstyle='round')
    # # Draw the decision boundary
    # x_values = np.array(plt.xlim())
    # y_values = -(w1 / w2) * x_values - (b / w2)
    # plt.plot(x_values, y_values, "--", color="black")  # 使用黑色虚线画出决策边界
...
def plot_training_half_moon (ax):
    ...
    # Scatter plot for each point in the dataset
    # for i, (a, b) in enumerate(zip(f1, f2)):
    #     if majority_minority[i] == 0:#majority实框
    #         plt.scatter(a, b, marker='o', color=y_colormap[y[i]], s=200)
    #     else:#minority虚框
    #         plt.scatter(a, b, marker='o', color=y_colormap[y[i]], s=200)
    #     #plt.scatter(a, b, marker='o', color=y_colormap[y[i]], s=200)
    ...
    for i, (a, b) in enumerate(zip(f1, f2)):
        if majority_minority[i] == 'Female':  # majority实框
            ...
            ax.scatter(a, b, marker='o', color=y_colormap[y[i]], s=200, edgecolors='black', linestyle='--', alpha=alpha1)  # 先画散点
    ...
    for i, (a, b) in enumerate(zip(f1, f2)):
        if i in indices:
            if majority_minority[i] == 'Female':
                ...
                plt.scatter(a, b, marker='X', color=y_colormap[y[i]], s=500, edgecolors='black', linestyle='--', alpha=alpha1)
            #plt.scatter(a, b, marker='X', color=y_colormap[y[i]], s=500, edgecolor='black')
...
plot_functions = [plot_training_toy_Utility, plot_test_toy_utility, plot_influence_inner_product_LR_Utility, 
                  #plot_training_toy_Fariness, plot_test_toy_fairness, plot_influence_inner_product_LR_Fairness, 
                  #plot_training_toy_Robutness, plot_advtest_toy, plot_influence_inner_product_LR_Robutness, 
...
fig.legend(handles=legend_elements, loc='upper center', bbox_to_anchor=(0.5, 1.1), ncol=3,
           fontsize='large', title="Legend", title_fontsize='large', handlelength=2, handletextpad=1, labelspacing=1.2)
