...
class Dataset():
    ...
    Sensitive feature is not excluded from data - except for toy and half_moons data which will be handled later
    """
    ...
    def __init__(self, name, df, target_feat, sensitive_feat, l2_reg, test_df=None, categorical_feat=None,
        ...
        if name == "Toy" or name == 'Half_Moons':
            ...
            #Setting validation set as test set
...
class Half_Moons(Dataset):

    def __init__(self):
        meta = json.load(open("./data/half_moons/meta.json"))
        meta["categorical_feat"] = meta["categorical_feat"].split(",")
        ...
        train = pd.read_csv(meta["train_path"], names=column_names, skipinitialspace=True,
                            na_values=meta["na_values"])
        #print 最大最小值
        #print("train.max()",train.max(),"train.min()",train.min())
        test = pd.read_csv(meta["test_path"], names=column_names, skipinitialspace=True,
                           na_values=meta["na_values"])        


        super(Half_Moons, self).__init__(name="Half_Moons", df=train, test_df=test, **meta, shuffle=False)
