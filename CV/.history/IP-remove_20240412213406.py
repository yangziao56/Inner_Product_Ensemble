...
for _ in range(args.repeat):
    ...
    for __ in range(args.ensemble_size):
        ...
        # 假设已有模型和数据加载器
        model = ...  # 模型应已初始化并配置到适当的设备上
        temp_dl_train = DataLoader(...)  # 数据加载器
        ...
        # 定义一个函数计算梯度
        def compute_grads(params, buffers, images, labels):
            logits = functional_call(model, (params, buffers), (images,))
            loss = F.cross_entropy(logits, labels)
            return grad(loss, params.values(), create_graph=True)

        # 使用vmap向量化计算梯度
        compute_grads_batch = vmap(compute_grads, in_dims=(None, None, 0, 0))

        # 初始化累加器
        #IP_sum = 0

        # 迭代数据加载器
        for images, labels in tqdm(temp_dl_train):
            images = images.to(device)
            labels = labels.to(device)
            
            # 计算每个样本的梯度
            grads_per_sample = compute_grads_batch(params, buffers, images, labels)
            
            # 累加梯度（如果有需要的话）
            grads = torch.cat([g.flatten() for g in grads_per_sample])
            
            # 计算内积
            inner_products = torch.dot(grads, average_grads_val)
            
            # 累加内积结果
            IP_sum += inner_products
