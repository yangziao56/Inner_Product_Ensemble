...
class BasicBlock_vmap(nn.Module):
    expansion = 1

    def __init__(self, in_planes, planes, stride=1):
        super(BasicBlock_vmap, self).__init__()
        self.conv1 = nn.Conv2d(in_planes, planes, kernel_size=3, stride=stride, padding=1, bias=False)
...
class ResNet_dropout(nn.Module):
    def __init__(self, block, num_blocks, num_classes=10, dropout_rate=0.01):
        super(ResNet_dropout, self).__init__()
        self.in_planes = 64

        self.dropout1 = nn.Dropout2d(dropout_rate)  # input dropout
        print('input dropout rate:', dropout_rate)
        self.conv1 = nn.Conv2d(3, 64, kernel_size=3, stride=1, padding=1, bias=False)
        self.bn1 = nn.BatchNorm2d(64, track_running_stats=False)
        self.dropout2 = nn.Dropout2d(dropout_rate)  # 在第一个卷积和第一个残差块之间加入Dropout
        ...
        self.avgpool = nn.AdaptiveAvgPool2d((1, 1))
        self.dropout3 = nn.Dropout(dropout_rate)  # 在最后的全连接层之前加入Dropout
        self.linear = nn.Linear(512*block.expansion, num_classes)
    ...
    def _make_layer(self, block, planes, num_blocks, stride, dropout_rate):
        ...
        for stride in strides:
            layers.append(block(self.in_planes, planes, stride))
            layers.append(nn.Dropout2d(dropout_rate))  # 在每个残差块的输出后添加 Dropout
            self.in_planes = planes * block.expansion
        return nn.Sequential(*layers)

    def forward(self, x):
        #x = self.dropout1(x.permute(1, 0, 2, 3)).permute(1, 0, 2, 3) 
        out = F.relu(self.bn1(self.conv1(x)))
...
def ResNet34_dropout(num_classes, dropout_rate=0.1):
    return ResNet_dropout(BasicBlock_vmap, [3,4,6,3],num_classes=num_classes, dropout_rate=dropout_rate)
# test()
def ResNet34_vmap(num_classes):
    return ResNet(BasicBlock_vmap, [3,4,6,3],num_classes=num_classes)
