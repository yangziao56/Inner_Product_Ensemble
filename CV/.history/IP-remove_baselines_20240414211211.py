...
parser.add_argument('--grad_batch_size', type = int, default = 32) #128
...
parser.add_argument('--n_epoch', type=int, default=2)#100
...
if __name__ == '__main__':
    ...
    for images, labels,_ in tqdm(temp_dl_train):
        ...
        # 计算每个样本的梯度，但只包括最后一层 'linear'
        grads_per_sample = [
            torch.cat([
                ft_per_sample_grads['linear.weight'][i].flatten()
    ...
    grads = torch.cat(grads).cpu().detach().numpy()
