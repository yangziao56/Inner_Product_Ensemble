...
for _ in range(args.repeat):
    ...
    for __ in range(args.ensemble_size):
        ...
        for images, labels,_ in tqdm(temp_dl_train):
            ...
            batch_size = next(iter(ft_per_sample_grads.values())).shape[0]
            # 为每个样本初始化一个梯度列表
            grads_per_sample = [torch.cat([g[i].flatten() for g in ft_per_sample_grads.values()]) for i in range(batch_size)]
            # 将列表转换为一个Tensor
            total_grads_per_sample = torch.stack(grads_per_sample)
            print(total_grads_per_sample.shape)  
