...
for _ in range(args.repeat):
    ...
    for __ in range(args.ensemble_size):
        ...
        # 定义一个函数计算梯度和内积
        def compute_grads_and_ip(params, buffers, image, label, avg_grads):
            # 确保 image 是四维的
            if image.dim() == 3:
                image = image.unsqueeze(0)  # 添加批次维度
            logits = functional_call(model, (params, buffers), (image,))
            loss = F.cross_entropy(logits, labe.unsqueeze(0))
            grads = grad(loss, params.values())
            flattened_grads = torch.cat([g.flatten() for g in grads])
            return torch.dot(flattened_grads, avg_grads)

        # 使用vmap向量化梯度和内积计算
        compute_ip_batch = vmap(compute_grads_and_ip, in_dims=(None, None, 0, 0, None))
        ...
        for images, labels,_ in tqdm(temp_dl_train):
            ...
            # 计算批次内每个样本的内积
            ips = compute_ip_batch(params, buffers, images, labels, average_grads_val)
