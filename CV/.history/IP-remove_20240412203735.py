...
for _ in range(args.repeat):
    ...
    for __ in range(args.ensemble_size):
        ...
        def compute_batch_grads(images, labels, model, device, last_layer=False):
            images = images.to(device)
            labels = labels.to(device)
            model.zero_grad()
            logits = model(images)
        ...
        def compute_IP(model, data_loader, average_grads_val, device, args):
            IP = []
            for images, labels, _ in tqdm(data_loader):
                grads = compute_batch_grads(images, labels, model, device, args.last_layer)
                inner_product = torch.dot(grads, average_grads_val)
                IP.append(inner_product.item())

            IP = torch.tensor(IP)
            return IP.sum()

        # 示例使用
        # 假设 average_grads_val 是预先计算好的平均梯度
        # model, data_loader, args 需要根据实际情况提前定义

        IP_sum = compute_IP(model, temp_dl_train, average_grads_val, device, args)
