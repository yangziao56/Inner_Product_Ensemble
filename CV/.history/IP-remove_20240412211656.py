...
for _ in range(args.repeat):
    ...
    for __ in range(args.ensemble_size):
        ...
        import torch
        import torch.nn.functional as F
        from torch.autograd.functional import vmap

        def compute_grad_and_loss(image, label, model, average_grads_val, last_layer=False):
            image = image.unsqueeze(0)  # Add batch dimension for single sample processing
            ...
            if last_layer:
                ...
                grads = torch.cat([param.grad.flatten() for param in model.parameters()])
            inner_product = torch.dot(grads, average_grads_val)
            return inner_product, loss.item()

        # Using vmap to compute gradients and losses for all samples in the batch simultaneously
        def compute_IP_with_vmap(model, data_loader, average_grads_val, device, args):
            batch_inner_product = []
            batch_losses = []
            ...
            for images, labels, _ in data_loader:
                ...
                # Applying vmap over compute_grad_and_loss function
                inner_products, losses = vmap(compute_grad_and_loss, in_dims=(0, 0, None, None, None))(
                    images, labels, model, average_grads_val, args.last_layer
                )

                batch_inner_product.extend(inner_products.tolist())
                batch_losses.extend(losses.tolist())

            total_inner_product = sum(batch_inner_product)
            mean_loss = sum(batch_losses) / len(batch_losses)
            return total_inner_product, mean_loss

        # Example usage
        device = torch.device("cuda" if torch.cuda.is_available() else "cpu")
        model.to(device)
        average_grads_val = torch.randn(model.parameters().__next__().size(), device=device)  # Example gradient vector
        # Ensure that temp_dl_train, model, and args are defined

        total_IP, average_loss = compute_IP_with_vmap(model, temp_dl_train, average_grads_val, device, args)
        print(f"Total Inner Product: {total_IP}, Average Loss: {average_loss}")
